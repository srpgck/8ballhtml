var GameGui = function () {
  ...
  btn_ball.onclick = function () {
    eightballgame.hitButtonClicked(
      Number(document.getElementById("range_strength").value)
    );
  };
  if (debug)
    document
      .getElementById("fps_stats_container")
...
GameGui.prototype.setupGameHud = function () {
  ...
  this.show(document.getElementById("pls"));
};
...
GameGui.prototype.play8BallClicked = function () {
  let p1;
  let p1i = prompt("Please enter player1 name:", "Player1 Name");
  if (p1i == null || p1i == "") {
    p1 = "Null";
  } else {
    p1 = p1i;
  ...
  let p2;
  let p2i = prompt("Please enter player2 name:", "Player2 Name");
  if (p2i == null || p2i == "") {
    p2 = "Null";
  } else {
    p2 = p2i;
  }
  const p1es = document.getElementById("p1s")
  const p2es = document.getElementById("p2s")

  p1es.innerText = "Player 1 ("+p1+")"

  p2es.innerText = "Player 2 ("+p2+")"
...
GameGui.prototype.updateBalls = function (ballArr, p1side, p2side) {
  ...
  document
    .getElementsByClassName(p1side == "solid" ? "player1" : "player2")[0]
    .replaceChild(
      elem,
      document.getElementsByClassName(
        p1side == "solid" ? "player1" : "player2"
      )[0].children[1]
    );
  ...
  document
    .getElementsByClassName(p1side == "striped" ? "player1" : "player2")[0]
    .replaceChild(
      elem,
      document.getElementsByClassName(
        p1side == "striped" ? "player1" : "player2"
      )[0].children[1]
    );
};
